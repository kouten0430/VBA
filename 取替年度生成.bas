Sub 次回取替年度生成()
    ...
    For i = 行始 + 1 To 行終
        ...
        ElseIf Cells(i, テキスト).Value = "遮断器" And Cells(i, 構造).Value = "ガス" And (Cells(i, 投入).Value = "空気" Or Cells(i, 開放).Value = "空気") Then
            Cells(i, 列終 + 1).Value = 36
        ElseIf Cells(i, テキスト).Value = "遮断器" And Cells(i, 構造).Value = "ガス" Then
            Cells(i, 列終 + 1).Value = 48
        ElseIf Cells(i, テキスト).Value = "遮断器" And Cells(i, 構造).Value = "真空" And (Cells(i, 投入).Value = "空気" Or Cells(i, 開放).Value = "空気") Then
            Cells(i, 列終 + 1).Value = 30
        ...
        ElseIf Cells(i, テキスト).Value = "遮断器" And Cells(i, 構造).Value = "真空" And Cells(i, 設置形態).Value = "ＥＶＣＢ内蔵" Then
            Cells(i, 列終 + 1).Value = 48
        ElseIf Cells(i, テキスト).Value = "遮断器" And Cells(i, 構造).Value = "真空" And Cells(i, 設置形態).Value = "キュービクル内蔵" And Cells(i, 型式).Value Like "VBBD*" Then   'レトロフィット不可能
            Cells(i, 列終 + 1).Value = 40
        ElseIf Cells(i, テキスト).Value = "遮断器" And Cells(i, 構造).Value = "真空" And Cells(i, 設置形態).Value = "キュービクル内蔵" And Not Cells(i, 型式).Value Like "VBBD*" Then
            Cells(i, 列終 + 1).Value = 50
    ...
    For i = 行始 + 1 To 行終
        ...
        If Cells(i, 列終 + 1).Value <> "" And Cells(i, tmp).Value <> "" Then
            If Month(Cells(i, tmp).Value) < 4 Then
                Cells(i, 列終 + 2).Value = Year(Cells(i, tmp).Value) - 1 + Cells(i, 列終 + 1).Value
            Else
                Cells(i, 列終 + 2).Value = Year(Cells(i, tmp).Value) + Cells(i, 列終 + 1).Value
            End If
        End If
    Next i
