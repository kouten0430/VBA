Sub ラベルをすべて繰り返し並び替え()
    ...
    Cells.Find("氏名/組織名", LookAt:=xlWhole).Sort Cells.Find("開始時刻", LookAt:=xlWhole), xlAscending, Cells.Find("予定詳細", LookAt:=xlWhole), , xlAscending, , , xlGuess
    
    Cells.Find("氏名/組織名", LookAt:=xlWhole).CurrentRegion.RowHeight = Rows(行始).RowHeight
