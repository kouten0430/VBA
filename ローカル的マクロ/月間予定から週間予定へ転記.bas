Sub 月間予定から週間予定へ転記()
    ...
    '休み関連以外の転記元の内容は、「複数の区切り」変数の文字列で区切って複数入力可　※二つまで
    '半自動の入力支援です
    'ローカル的なマクロです
    Dim 日付行 As Long
...
skip:
        ...
        ElseIf InStr(内容, 複数の区切り) <> 0 Then   '内容が複数の場合の処理
            複数の内容 = Split(内容, 複数の区切り, 2)
            
            For j = 0 To UBound(複数の内容)
                If 複数の内容(j) Like "*巡視*" Or 複数の内容(j) Like "*教育*" Or 複数の内容(j) Like "*研修*" Or 複数の内容(j) Like "*講習*" Or 複数の内容(j) Like "*訓練*" Then   '作業と巡視他で転記先のセルを変える
                    転記先背景色 = 巡視色
                Else
                    転記先背景色 = 作業色
                End If
                ...
                For i = 1 To 転記先行終
                    ...
                    And Workbooks(転記先ワークブック名).Sheets(転記先シート名).Cells(i, 転記先内容列).Interior.Color = 転記先背景色 Then
                        ...
                        If myRange.Interior.Color <> 13408767 Then
                            myRange.Interior.Color = 13408767  '転記に１回成功したらセルをピンクにする
                        Else
                            myRange.Interior.Color = 65535  '転記に２回成功したらセルを黄色にする
                        End If
        ...
        Else    '内容が単数の場合の処理
            If 内容 Like "*巡視*" Or 内容 Like "*教育*" Or 内容 Like "*研修*" Or 内容 Like "*講習*" Or 内容 Like "*訓練*" Then   '作業と巡視他で転記先のセルを変える
                転記先背景色 = 巡視色
            Else
                転記先背景色 = 作業色
            End If
