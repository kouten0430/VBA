Sub 技術事務の削除または復活単日版()
    'ローカル的なマクロです
    Dim 列始 As Integer
    Dim 列終 As Integer
    Dim 日付列 As Long
    Dim 内容列 As Long
    Dim 行終 As Long
    Dim 単日 As String
    Dim 列 As Integer
    Dim 行 As Long
    Dim 日付 As String
    Dim i As Long
    Dim 削除 As Long
    Dim 復活 As Long
    
    列始 = Cells.Find("A", LookAt:=xlWhole).Column
    列終 = Cells.Find("B", LookAt:=xlWhole).Column
    日付列 = 1
    内容列 = 6
    
    行終 = Cells(Rows.Count, 日付列).End(xlUp).Row
    単日 = Year(Cells(Selection.Row, 日付列).Value) & Month(Cells(Selection.Row, 日付列).Value) & Day(Cells(Selection.Row, 日付列).Value)
    
    Application.ScreenUpdating = False
    
    For 列 = 列始 To 列終
        For 行 = 1 To 行終
            If Cells(行, 内容列).Value Like "技術事務*" And TypeName(Cells(行, 日付列).Value) = "Date" Then
                日付 = Year(Cells(行, 日付列).Value) & Month(Cells(行, 日付列).Value) & Day(Cells(行, 日付列).Value)
                
                If 日付 <> 単日 Then GoTo continue
                
                For i = 行 + 1 To 行終
                    If TypeName(Cells(i, 日付列).Value) = "Date" Then
                        If 日付 = Year(Cells(i, 日付列).Value) & Month(Cells(i, 日付列).Value) & Day(Cells(i, 日付列).Value) Then
                            If Cells(i, 列).Value <> "" And Not Cells(i, 内容列).Value Like "*日直*" And Not Cells(i, 列).Value Like "*[pPｐＰaAａＡfFｆＦ]*" Then
                                If Cells(行, 列).Value <> "" Then
                                    Cells(行, 列).ClearContents
                                    削除 = 削除 + 1
                                End If
                                
                                GoTo skip
                                
                            End If
                        Else
                            Exit For
                            
                        End If
                    End If
                Next i
                
                If Cells(行, 列).Value = "" Then
                    Cells(行, 列).Value = "○"
                    復活 = 復活 + 1
                End If
                
skip:
                
            End If
            
continue:

        Next 行
        
    Next 列
    
    Application.ScreenUpdating = True

    MsgBox "削除：" & 削除 & vbCrLf & "復活：" & 復活

End Sub