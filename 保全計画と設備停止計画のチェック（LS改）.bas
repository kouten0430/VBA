                ...
                For k = 年停行始 To 年停行終
                    ...
                    If Workbooks(検索先ワークブック名).Sheets(1).Rows(k).Hidden = False Then  '可視セルのみ処理を行う
                        ...
                        If 実施個所 = 電気所等名称 And 作業内容 Like "*" & 切出し文字 & "*" Then    '実施個所と作業内容が一致していれば着色
                            切始 = InStr(作業内容, 切出し文字)
                            For j = 切始 To Len(作業内容)
                                If Mid(作業内容, j, 1) Like "[!０-９Ａ-Ｚ－]" Then
                                    切出し文字弐 = Mid(作業内容, 切始, j - 切始)
                                    Exit For
            
                                End If
                            Next j
                            
                            If Len(切出し文字) = Len(切出し文字弐) Then
                        ...
                        ElseIf 実施個所 = 電気所等名称 And 要求時注釈 Like "*" & 切出し文字 & "*" Then
                            切始 = InStr(要求時注釈, 切出し文字)
                            For j = 切始 To Len(要求時注釈)
                                If Mid(要求時注釈, j, 1) Like "[!０-９Ａ-Ｚ－]" Then
                                    切出し文字弐 = Mid(要求時注釈, 切始, j - 切始)
                                    Exit For
