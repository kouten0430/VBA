Sub 日付データを好きな書式で文字列に変換()
    ...
    "16:m/d h:mm" & vbCrLf & "17:h:mm" & vbCrLf & "18:yyyy（年度）" & vbCrLf & vbCrLf, Type:=1)
        If TypeName(n) = "Boolean" Or n < 1 Or n > 18 Then
            Exit Sub
        End If
    ...
    For Each myRange In Selection.SpecialCells(xlCellTypeVisible)   '可視セルのみに処理を行う
        If myRange.Address = myRange.MergeArea(1).Address Then   '結合セルの場合は左上のセルのみ処理を行う
            ...
            Case 18
                If Month(myRange.Value) < 4 Then
                    myRange.Value = "'" & Year(myRange.Value) - 1
                Else
                    myRange.Value = "'" & Year(myRange.Value)
                End If
