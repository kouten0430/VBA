Sub n行挿入する式コピー版()
    ...
    If n >= 1 And n <= 1000000 Then
        Rows(Selection(1).Row + 1 & ":" & Selection(1).Row + 1 + n - 1).Insert xlShiftDown, xlFormatFromLeftOrAbove
        
        For Each myRange In Selection
            If myRange.Row = Selection(1).Row Then
                Range(Cells(myRange.Row + 1, myRange.Column), Cells(myRange.Row + 1 + n - 1, myRange.Column)).FormulaR1C1 _
